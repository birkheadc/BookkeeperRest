...
public class ExpenseCategoryRepository : CrudRepositoryBase, IExpenseCategoryRepository
{
    public ExpenseCategoryRepository(IWebHostEnvironment env, IConfiguration configuration) : base(env, configuration, "expense_categories", "CREATE TABLE expense_categories ( name VARCHAR(255) DEFAULT '__blank__' NOT NULL PRIMARY KEY, isDefault BOOL DEFAULT 0 NOT NULL)")
    {
    ...
    public void AddCategoriesByExpenses(IEnumerable<Expense> expenses)
    {
        ...
        {
            if (DoesCategoryExistByName(expense.Category) == false)
            {
